...
class User{
    ...
    static showAll(){
        const all = deal.readFromJson()
        all.forEach(all => {
            console.log(`id:${all.id}  name:${all.name}  age:${all.age} 
             email:${all.email}  status:${all.status}`)
        })
    ...
    static showSingle(userId){
        ...
        all.forEach(all=> {
            if(userId === all.id)
            {
                console.log(`id:${all.id}  name:${all.name}
                  age:${all.age}  email:${all.email}  status:${all.status}`)
            }
        })
    }
